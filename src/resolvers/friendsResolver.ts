import { Resolver, Mutation, UseMiddleware, Arg, Query, Ctx } from 'type-graphql'
...
export class FriendsResolver {
	...
	async fetchRequestee(@Ctx() { payload }: MyContext): Promise<FriendRequest[]> {
	...
	@Query(() => [FriendRequest])
	@UseMiddleware(isAuth)
	async fetchRequestor(@Ctx() { payload }: MyContext): Promise<FriendRequest[]> {
		const requests = await getConnection()
			.getRepository(User)
			.createQueryBuilder('users')
			.leftJoinAndSelect('users.requests', 'requests')
	...
	@Query(() => [Friend])
	@UseMiddleware(isAuth)
	async fetchFriends(@Ctx() { payload }: MyContext): Promise<Friend[]> {
	...
	@Mutation(() => Boolean)
	@UseMiddleware(isAuth)
	async requestFriend(@Arg('friend') friend: string, @Ctx() { payload }: MyContext): Promise<boolean> {
		try {
			const user = await User.findOne(payload?.userId)
